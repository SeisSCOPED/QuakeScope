"""The DocumentDB backend: the 2025 catalog's storage layer.

Split out of `utils` so that importing the pure helpers there does not drag in
`pymongo`. A 2026 picking campaign never touches this module - v3 keeps all
state in S3 (`s3_state`) and `worker.S3StateAdapter` implements the small slice
of this interface the picking path uses. Association and the 2025 CLI still need
